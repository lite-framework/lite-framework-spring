...
import cc.lite.framework.type.DebugControl;
import cc.lite.framework.type.IResponseCode;
import cc.lite.framework.utils.*;
...
public abstract class CommonBase implements DebugControl {
    ...
    // ------------------------------- template ---------------------------
    protected CommonResponse exec(Callback callback) {
        ...
        try {
            ...
            if (false)
                err(CommonResponseCode.SYSTEM_ERROR);
            ...
            if (causeBy instanceof io.lettuce.core.RedisException) {
                response.setCode(CommonResponseCode.REDIS_ERROR.getCode());
            } else if (causeBy instanceof org.hibernate.HibernateException) {

                if (StringUtils.contains(message, Def.IDEM_MARK)) {
                    response.setCode(CommonResponseCode.DUP_REQUEST.getCode());
                } else {
                    response.setCode(CommonResponseCode.JPA_ERROR.getCode());
                }
            } else if (causeBy instanceof org.springframework.amqp.AmqpException) {
                response.setCode(CommonResponseCode.RABBIT_MQ_ERROR.getCode());
            } else {
                response.setCode(CommonResponseCode.SYSTEM_ERROR.getCode());
            }
